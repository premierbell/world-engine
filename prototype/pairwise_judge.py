...
_PROMPTS = {
    ...
    # Experiment #56 Stage 2 신설 - RQ10-1 Stage 1 Pilot에서 실제 Ground
    # Truth(사용자가 함께 보관한 그룹)가 "같은 주제"가 아니라 "같은 용도로
    # 나중에 참고할 묶음(task bundle)"이었다는 게 드러난 뒤, 질문 자체를 그
    # Ground Truth와 정렬시키기 위한 프롬프트. "같은가/관련있는가"가 아니라
    # 사용자의 미래 행동(다시 찾아볼 가능성)을 직접 묻는다.
    "retrieval": (
        "다음 두 스크랩 요약을 비교해서, 이걸 저장한 사용자가 나중에 이 둘을 "
        "함께 다시 찾아볼 가능성이 얼마나 높은지 0.0~1.0 사이의 점수로 "
        "평가하라. 둘이 같은 주제나 개념을 다루는지는 중요하지 않다 - 완전히 "
        "다른 주제라도 같은 상황/목적(예: 같은 여행을 준비할 때, 같은 물건을 "
        "살 때)에서 함께 참고하게 될 가능성이 높으면 높은 점수를 줘라. 점수 "
        "숫자 하나만 출력하라(예: 0.85). 다른 설명은 절대 덧붙이지 마라.\n\n"
        "스크랩 A: {a}\n스크랩 B: {b}"
    ),
}
