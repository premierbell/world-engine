...
### Finding: 나무위키 IP 편집 제한 안내가 본문에 섞여 들어옴 (2026-08-11, 관찰만)

Topic 이름 AI 제안(PR #128) 실사용 중 "여행" Island 후보 그룹이 "나무위키
저작권"으로 제안된 걸 계기로 나무위키 스크랩 34건을 전수 스캔 - 저작권
boilerplate 오염은 기존에 알던 2건(제주신화월드/주상절리대, PR #129에서
`isValid()` 수정)뿐이었고 나머지 32건은 정상이었다.

PR #129 수정 후 두 스크랩을 삭제하고 재스크랩하는 과정에서 새로운 패턴을
하나 더 확인: 제주신화월드 페이지를 다시 스크랩하니 `DIRECT_EXTRACTION`으로
성공 처리됐지만, content 앞부분에 나무위키의 **IP 편집 제한 안내
배너**("이용중인 IP는 문제가 자주 발생하거나 공공 IP 대역이므로 로그인이
필요합니다... 현재 이용 중인 IP는 문서 훼손이 잦아 비로그인 편집이
제한되어 있습니다...")가 실제 본문("여러 시설들이 위치해 있으며 식당,
제주 프리미엄 전문점이 입점해 있다...") 앞에 그대로 섞여 저장됨.

**Type D(위 표/사례들)와 구분되는 점**: Type D는 decoy가 실콘텐츠를
완전히 대체하는 경우들이었는데(63빌딩/KT위즈샵/나이키/삼성바이오/대원제약
전부 decoy 단독), 이번 건은 안내 배너 + 실콘텐츠 + 저작권 문구가
한 블록으로 같이 잡힌 것 - 실콘텐츠는 보존돼 있어 치명적 실패는 아님.

**지금 필터를 추가하지 않기로 한 이유**:
- 저작권 boilerplate는 여러 페이지에서 반복되는 고정 문구라 일반화된
  제거 규칙(키워드 밀도 500자 윈도우)의 근거가 있었음.
- 이 IP 편집 제한 안내는 나무위키의 **IP별 편집 제한 정책**에 따라
  상황적으로 삽입되는 UI 문구라, 등장 형태(위치/문구)가 페이지마다
  달라질 가능성이 있고 지금은 표본이 1건뿐 - 여기서 바로 "이 문구가
  나오면 제거"를 추가하면 사이트별 예외를 하나씩 계속 쌓는 구조가
  된다.

**결정**: 지금은 기록만 하고 PR 범위에 넣지 않는다. 동일 패턴이 반복
관찰되거나(예: WARN 로그 관찰 방식과 같은 논리로 표본이 쌓이면) 요약
품질에 실제로 영향을 주는 사례가 더 나오면 그때 boilerplate 제거
규칙이나 추출 영역 개선을 검토한다 - 이 문서 전체가 지켜온 "발견 →
기록 → 반복 증거 확보 → 일반화" 순서를 그대로 적용.
