...
- **Intra-Island cosine variance** - 같은 Island 안 스크랩들의 embedding이
  서로 얼마나 흩어져 있는가. Finding V1-003("여행" Island 0.25~0.75
  편차)이 이미 이 신호가 실제 혼재 사례와 상관있다는 최초 증거.
...
이 문서는 원칙만 고정한다. 구현은 가장 작은 단위부터:

1. (다음 PR) Island 상세 화면에 두 신호(cosine variance, override rate)만
   노출 - 액션 버튼 없음. "이 숫자가 실제로 사람이 '이 Island가 좀
   이상한데?'라고 느끼는 것과 상관있는가"부터 실사용으로 확인한다.
2. 신호가 실제로 의미 있다는 근거가 쌓이면 수동 Topic 생성(사용자가
   스크랩을 직접 골라 Topic으로 묶기) 검토.
3. **AI는 Topic을 생성하지 않는다.** 위 두 단계가 실사용으로 검증된
   이후에도 AI가 할 수 있는 역할은 사용자가 검토할 후보 묶음(예: "일본
   / 유럽")을 제안하는 것까지다 - 그 이상은 하지 않는다.
