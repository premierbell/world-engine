...
1. **Mechanism Pairwise Judge로 Island 내부 Topic 후보를 생성한다.**
   AI가 하는 일은 여기까지 - 후보를 "찾아서 보여주는" 것. ✅ 완료(PR
   #80, Grouping 알고리즘은 Connected Components → Clique-safe로
   개선, PR #81).
2. **사용자는 후보를 승인/이름 변경/병합/삭제할 수 있다.** AI는 이
   결정에 관여하지 않는다. ✅ 완료 - 다만 "승인"은 별도 백엔드
   개념이 아니라, 후보 카드의 "이 후보로 채우기" 버튼이 아래 3번의
   생성 폼 체크박스를 미리 채워주는 UI 동작일 뿐이다(PR #83). 병합/
   삭제 액션은 아직 없음 - 필요성 근거가 쌓이면 추가.
3. **승인된 후보만 Topic이 된다.** 승인 전까지는 어떤 것도 확정되지
   않는다 - Scrap이 Island를 벗어나는 일도 없다(Design Principle
   "Island는 정체성으로 유지된다" 그대로). ✅ 완료(PR #82) - Topic에
   한번 배정된 스크랩은 후보 생성/선택 목록에서 제외된다(PR #83).
4. Topic 이름은 AI가 초안을 제안할 수 있지만(V0 `label_generator.py`의
   presentation 계층 역할), 최종 이름은 항상 사용자가 정한다. **아직
   미구현** - 사용자가 직접 입력하는 지금 방식이 불편하다는 근거가
   아직 없어서 보류(증거 없이는 안 만든다).
...
**1~3번이 완성되면서 V2의 첫 마일스톤이 완료됐다** - V0가 실패한
"AI가 자동으로 Topic을 만든다"를 "AI는 제안, 사용자는 확정"으로
재해석해서 실제로 동작하는 루프를 만들었다.

**남은 것과 보류 이유**:
- 병렬화(24개 Island 기준 4분 4초 실측) - 그 24개 데이터가 이후
  로컬 DB 사고로 사라져서(별도 기록 참고) 지금은 개선을 검증할
  큰 데이터가 없다. 실사용으로 다시 쌓이면 재검토.
- Topic 이름 AI 제안(4번) - 위와 같은 이유로 보류.
- 이후 한동안은 실제로 써보면서 "AI 후보가 자연스러운가/Topic을
  만들고 싶은 순간이 오는가"를 정성적으로 관찰하는 게 다음 방향을
  정하는 데 새 기능 추가보다 더 도움이 될 것으로 판단.
