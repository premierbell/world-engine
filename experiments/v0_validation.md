...
## ExtractionQualityEvaluator 보일러플레이트 검출 가드레일

패션/운동 카테고리 실사용 테스트 중 쇼핑몰 상품 페이지 4건에서
같은 패턴을 확인 - 길이 조건(50자)은 넘지만 실제로는 반품/배송/결제
안내 같은 "운영 정보"가 본문으로 잘못 추출됨. 서로 다른 템플릿
2종(KT위즈샵/LG트윈스샵의 "교환 및 반품" 계열, SSG백화점/SSG몰의
"구매혜택/배송정보" 계열)에서 독립적으로 나타나 우연이 아니라고
판단. 실제 피해 사례: SSG 와이셔츠 상품 페이지가 반품 안내문 때문에
"야구" Island와 0.90으로 오추천됨(둘 다 무관한 반품 안내문일 뿐인데
서로 유사하다고 판단).

GPT 의견을 두 차례 필터링:
1. "Open Graph를 먼저 시도" 제안 - 실제로는 이미 존재하는
   `openGraphFallback()`과 기능적으로 동일하다는 걸 두 URL의 실제
   og:description을 curl로 직접 확인해서 검증, 별도 구현 불필요로
   판단.
2. 전용 `CommerceStrategy` 제안 - GitHub/YouTube와 달리 커머스는
   도메인이 무한해서 "이 URL이 커머스인가"를 안정적으로 판별하는
   라우팅 문제가 먼저 풀려야 함. 이 문제가 안 풀린 채로 만들면
   결국 도메인 화이트리스트로 사례별 대응하게 되는데, 이는 GPT
   스스로도 경계했던 "케이스별 특수 규칙"이 문구에서 도메인으로
   옮겨간 것뿐 - 지금 착수하지 않기로 결정.

최종 구현: 운영 어휘("배송/교환/반품/환불/쿠폰/적립/상품번호/모델번호/
결제/무료배송/택배") 11개 중 3개 이상이 본문 앞 500자 안에 몰려있으면
보일러플레이트로 판단해 기존 Open Graph fallback으로 넘김. 택배업
취준생의 자소서처럼 이 어휘들이 정당하게 나오는 글도 있을 수 있다는
정밀도 우려가 있었지만, 앞부분 밀집도 조건 덕에 자연스러운 글에서는
쉽게 3개 이상이 좁은 구간에 몰리지 않음 - 실제 정상 글 예시로 검증.

Round 1 KPI 재측정에서 성공률이 88%→84%로 떨어져 순간 회귀로
의심했으나, Failure Reason 세부 내역을 보니 원인은 `ROBOTS_BLOCKED`
1건→2건 증가(외부 사이트의 시점별 차단 정책 변화, 이번 변경과 무관)뿐이고
`EMPTY_CONTENT`(이 가드레일이 만들어낼 수 있는 오탐)는 0건 - 무관한
사이트 25개 전부에서 오탐 없음을 확인.

### Result
`ExtractionQualityEvaluator`에 운영 어휘 밀도 체크 추가(길이 체크와
같은 `isValid()` 안에서 처리, 호출부 변경 없음). 유닛 테스트 4건
추가, 전체 테스트 통과. 실제 재현: SSG 와이셔츠는 이제 진짜
og:description으로 개선되고, LG트윈스 상품은 거짓 "야구 0.90" 매칭
대신 정직하게 `EMPTY_CONTENT`로 실패 처리됨.

### Decision
`ExtractionQualityEvaluator` + 테스트 커밋. 문서는 "쇼핑몰 지원"이
아니라 "본문 대신 운영성 보일러플레이트가 선택되는 문제"로 일반화해서
`docs/content_extraction.md`에 기록, `CommerceStrategy`는 라우팅
문제가 풀리기 전까지 Future Idea로만 남김.

## V2 신호 검증 - cosine variance는 약함, Mechanism Pairwise Judge는 재현됨 (2026-07-28)

PR #78로 Island 상세 화면에 cosine variance/override rate를 실제
노출한 뒤, 사용자가 "여행" Island에 제주/부산 스크랩을 집중적으로
추가하며 "유의미한 값이 안 보이는 것 같다"고 리포트. 실제 Postgres
데이터(24개 스크랩, 제주 9/부산 12/기타 3)로 직접 검증.

### Evidence 1 - cosine variance는 혼재도만 반영, Topic 존재와는 약한 상관
제주-제주 평균 cosine similarity 0.491, 부산-부산 0.475, 제주-부산
0.360 - embedding 자체는 지역을 어느 정도 구분한다. 하지만 Island
전체의 pooled variance(0.0156)는 다른 Island와 비교했을 때 특별히
높지 않았다 - 하위 그룹이 알려지지 않은 "야구" Island가 오히려 더
높음(0.0236). 24x24 유사도 히트맵(제주/부산/기타로 정렬)에서도 두
블록이 미세하게 보이긴 했지만, 공식 사이트/기관 페이지(광안대로
교량소개, 해동용궁사 등)가 같은 지역 안에서도 유독 낮은 유사도를
보여 콘텐츠 장르 차이가 지역 차이만큼 강하게 신호를 흐렸다.

### Evidence 2 - Mechanism Pairwise Judge는 지역을 깨끗하게 구분, 장르에 안 흔들림
V0 `prototype/pairwise_judge.py`의 "Mechanism" objective(같은 구체적
하위 주제/대상을 다루는지 명시적으로 요구, Finding #012의 AUC
0.82~0.94 신호)를 재사용해 2x2 설계(지역 같음/다름 × 장르 같음/다름)
로 100쌍을 gpt-4o-mini로 채점:
...
다른 지역 쌍은 장르가 같아도(예: 제주닷컴 ↔ 광안대로 교량소개, 둘 다
공식 사이트) 예외 없이 0.000이었다 - 장르 혼입에 흔들리지 않고
지역을 정확히 구분했다. 같은 지역 안에서도 0.0~0.9로 점수가
갈렸는데(예: 제주 맛집 리스트 vs 한라산 등산 코스는 낮은 점수), 이건
Mechanism이 지역보다 더 세밀한 구체적 하위 주제까지 구분한다는
뜻으로, Topic 후보를 만들 때는 오히려 장점이다.

### Root Cause
cosine variance는 raw embedding의 전반적 분산이라 콘텐츠 장르(문체,
사이트 구조)처럼 주제와 무관한 요인에도 크게 반응한다. Mechanism
프롬프트는 "같은 구체적 대상을 다루는가"를 명시적으로 묻기 때문에
장르가 아니라 실제 내용으로 판단한다 - Finding #012(V0)가 왜 강한
신호였는지가 V1의 실제 데이터로 다시 확인된 것.

### Decision
`docs/v2_design.md` 갱신 - cosine variance는 "Island 건강도" 참고
지표로만 남기고 Topic 후보 탐색 신호에서 제외, Mechanism Pairwise
Judge를 Topic 후보 생성의 근거 신호로 채택. 로드맵도 "신호 → 수동
Topic 생성 → AI 그룹 제안" 3단계에서 "Mechanism Pairwise Judge로
후보 생성 → 사용자 승인/수정/병합/삭제 → 승인된 것만 Topic" 구조로
...
## Pairwise 병렬화 - OpenAI TPM 한도가 실제 병목이었다 (2026-07-30)

V2 핵심 루프(PR #80~86)가 완성된 뒤, 실측 기준(30초 이하 유지/1분
이상 병렬화)에 따라 `TopicCandidateService`의 pairwise 채점을
`CompletableFuture`+`ExecutorService`로 병렬화했다. 그룹핑 알고리즘/
threshold/API/UI는 그대로 - 실행 방식만 바꾸는 가장 작은 변경으로
설계.

### Setup
"여행" Island, 기존 Topic 매칭 + 새 그룹 pairwise 두 단계 모두 병렬화.
동일한 실제 데이터로 PARALLELISM 8 → 4 → 2 순서로 재시도.

### Result
- **PARALLELISM=8**: 12.3초 만에 끝났지만 **500 에러**(OpenAI 429 Too
  Many Requests, "TPM Limit 200000, Used 200000"). 빠른 게 아니라
  빠르게 실패한 것.
- **PARALLELISM=4 + 재시도(3회, 최대 6초 백오프)**: 재시도까지 다
  써도 여전히 429("Used 199075/200000") - 64.4초 만에 실패.
- **PARALLELISM=2**: 83.55초, **성공**(429 없음). 그룹/미분류 구성도
  이전 실행들과 일관됨(서울특별시/관광+서울여행코스 그룹, 나머지
  미분류).

### Observation
처음엔 "동시성이 너무 높다"로 착각했지만, 실제 원인은 **분당 토큰
소비 속도**였다 - 순차 실행(쌍당 ~0.9초)일 때는 총 요청이 자연히
1~3분에 걸쳐 퍼져서 분당 토큰 사용량이 우연히 한도 밑이었을 뿐,
병렬화는 같은 총 토큰량을 훨씬 짧은 시간에 몰아 쓰게 만들어서 TPM
한도(계정 기준 200,000/분)를 실제로 노출시켰다. 재시도(수 초 백오프)
만으로는 해결이 안 됐다 - 이 계정의 TPM 한도가 이 정도 규모(약
160건 호출)의 요청 자체를 거의 다 써버릴 만큼 빠듯했기 때문에,
동시성 자체를 낮춰서 소비 속도를 늦추는 것만이 실질적인 해결책이었다.

### Decision
`PARALLELISM=2`로 확정, `LlmPairwiseJudgeClient`에 429 재시도(짧은
백오프, 최대 3회) 추가 - 우연한 버스트에는 도움이 되고, 근본적인
완화는 낮은 동시성이 담당. 순차(2~3분대) 대비 실측 개선은 있지만
(83초), 8-way 동시성으로 기대했던 극적인 개선(수십 초→10여 초)은
이 계정의 TPM 한도 안에서는 달성 불가 - "병렬화하면 빨라진다"는
가정이 외부 API 요금제 제약 앞에서는 무조건 성립하지 않는다는 게
실측으로 확인됨. 더 공격적인 개선(토큰 사용량 자체를 줄이거나,
TPM 한도를 올리거나)은 지금 근거가 부족해 보류.

## Cosine pre-filter는 Topic 후보 생성에 안 맞는다 (2026-07-30)

O(n²) pairwise 호출량 자체를 줄이기 위해, Island 추천(V1)에서 이미
검증된 "cosine recall → LLM precision" 2단계 구조를 Topic 후보
생성에도 적용할 수 있는지 실험. 코드 적용 전에 먼저 상관관계부터
측정하자는 판단(cosine만 믿고 Topic을 만들려다 실패했던 V0 경험과
구조적으로 닮아 있어서 - 증거 없이 바로 적용하지 않음).

### Setup
"AI" Island(22개, 231쌍) 전체에 대해 cosine similarity(로컬 계산)와
Mechanism score(`LlmPairwiseJudgeClient`와 동일 프롬프트/모델
gpt-4o-mini, Java 앱 우회하고 Python(`openai` SDK)으로 직접 호출)를
모든 쌍에 대해 나란히 캡처.

### Result
- Pearson 상관계수(cosine, Mechanism) = **0.60** - 중간 수준.
- Mechanism ≥ 0.6인 실제 관련 쌍 40개 중 cosine이 가장 낮았던 건
  **0.384**(MCP 설명 글 ↔ OpenAI Agents SDK, Mechanism 0.70).
- 진짜 관련 쌍을 하나도 안 놓치는 안전한 cutoff는 cosine < 0.38
  정도인데, 이 구간에서 줄어드는 LLM 호출은 **19.5%뿐**. 0.40으로만
  올려도 바로 1건을 놓치기 시작(| threshold 0.30→4.8% 감소/0건 손실,
  0.35→11.3%/0건, 0.38→19.5%/0건, 0.40→24.2%/1건, 0.50→69.3%/13건 |).

### Observation
cosine과 Mechanism의 상관관계 자체는 존재하지만, "안전하게 잘라도
되는 구간"과 "실제로 호출을 크게 줄여주는 구간"이 겹치지 않는다.
Island 추천(Neutral objective, 넓은 관련성)에서는 cosine recall이
잘 먹혔지만, Topic 후보 생성(Mechanism objective, "같은 구체적
대상인가")은 애초에 cosine이 못 잡는 걸 잡으라고 만든 지표라서, cosine
사전 필터를 걸면 그 지표가 못 잡는 부분(MCP↔Agents SDK 같은 사례)을
먼저 잘라내는 모순이 생긴다.

### Decision
Topic 후보 생성에는 cosine pre-filter를 도입하지 않는다. 병렬화
(PARALLELISM=2)는 유지. O(n²) 자체는 Stage 1(기존 Topic 매칭)이
쌓일수록 Stage 2 대상 풀을 자연히 줄여주는 구조라 "Topic이 거의 없는
어린 Island가 급격히 커지는" 예외적 상황에서만 문제가 되는 것으로
보고, 지금은 추가 최적화를 보류 - 실사용에서 실제로 병목이 재확인되면
그때 재검토.

## Topic 후보 생성 - "잘게 쪼갠다"는 현상의 3가지 원인 분해 (2026-07-30)

"AI" Island(22개, Topic 없음 상태) 실사용 관찰에서 반복적으로 나온
"AI가 너무 잘게 쪼갠다"는 인상을, 사용자가 직접 같은 22개를 스스로
그룹핑(RAG/LLM/프롬프트/MCP/Agent/파인튜닝 6개 큰 범주)해본 뒤 실제
AI 후보(8개 그룹)와 나란히 비교해서 원인을 분해. 하나의 현상 뒤에
최소 세 가지 서로 다른 원인이 섞여 있음을 확인 - 각각 처방이 다르다.

### Finding: Topic granularity mismatch
Mechanism Judge는 "같은 구체적 대상/기법인가"를 정확히 판별하도록
설계돼 있는데(V0 Finding #012 근거), 사용자가 원하는 Topic 단위는
그보다 넓은 "관리 폴더" 단위다. 예: RAG를 사용자는 하나로 묶는데
AI는 "RAG 개념"(0.85)과 "RAG 실전/VectorDB"(0.85)로 나눔 - 점수
자체는 정상이라 알고리즘 오류가 아니라 목적 함수 차이. threshold
조정으로 해결할 문제가 아님.

### Finding: Clique-safe grouping recall loss
그리디 first-fit 처리 순서 때문에, 진짜 관련 있는 항목이 그룹에서
탈락하는 사례를 추가로 확인. 예: "LangGraph란 무엇이고 언제 쓰면
좋을까?"(velog)는 "AI 에이전트란"과 묶였지만, 같은 주제를 다루는
"LangGraph란 무엇인가요?"(IBM)는 아예 미분류로 남음 - 서로 다른
소스의 LangGraph 설명 글 두 개끼리도 못 묶인 것. PR #85에서 이미
"서울특별시/관광 ↔ 경복궁"으로 한 번 확인됐던 것과 같은 유형의
실패 모드가 재현됨.

### Finding: Pairwise false positive
"RAG란?(AWS)"와 "AI를 위한 프롬프트 엔지니어링 가이드(Google)"가
0.65로 묶임 - 사용자 기준으론 둘 다 아닌 애매한 오매칭. 발생 빈도가
낮아 보이지만 이런 사례가 쌓이면 후보 품질 자체에 대한 신뢰가
떨어진다.

### Decision
지금 당장 grouping 알고리즘을 갈아엎지 않는다 - LLM pairwise
신호 자체(Mechanism judge)는 이미 검증된 자산이고, 문제는 신호가
아니라 "그 신호를 사람이 원하는 단위로 어떻게 묶어서 보여줄지"에
있다는 게 이번 비교로 명확해졌기 때문. Finding
"granularity mismatch"를 푸는 도구(여러 후보를 합쳐 하나의 Topic으로
만드는 기능)는 PR #88에서 이미 구현·검증까지 끝나 있음(RAG 개념+
RAG 실전을 합쳐 "RAG" Topic으로 만든 사례가 이 실험의 예시와 정확히
같은 케이스) - 새로 만들 필요 없이, 이 도구가 다른 도메인에서도
충분한지 검증하는 게 다음 단계. 다음 실험은 AI/LLM처럼 아직 확신
없는 도메인이 아니라 사용자가 강하게 판단할 수 있는 도메인(개발
CS 우선, 이후 여행/바이오)에서, 이번처럼 AI 결과를 먼저 보여주지
않고 블라인드로 그룹핑부터 받은 뒤 비교하는 방식으로 진행한다.

## "개발 CS" 블라인드 그룹핑 - Finding A/C 재정의 (2026-07-30)

위 실험의 다음 단계로, 확신 있는 도메인에서 재검증. "개발" Island
(10개, Topic 없음)의 스크랩 10개 제목만(AI 결과 숨김) 먼저 보여주고
사용자가 직접 그룹핑하게 한 뒤, 실제 `/islands/1/topic-candidates`
호출 결과 및 10개 전체 45쌍의 raw Mechanism 점수(Java 앱 우회, 직접
캡처)와 비교했다.

### Setup
사용자 그룹핑: 면접(35 backend-interview-question repo, 36, 38, 39,
41, 42) / 알고리즘(45, 49) / 판단 보류(2, 3 - Claude가 예전에 넣은
GitHub repo라 본인이 뭘 스크랩한 건지 모른다고 명시).

### Result
- 실제 앱 출력: 그룹1[2,38]=0.65, 그룹2[3,39]=0.90, 그룹3[35,45]=0.60,
  나머지(36,41,42,49) 미분류. "면접" 6개 중 3개가 흩어지고, 진짜
  면접 항목끼리는 하나도 안 묶임.
- raw Mechanism 45쌍 중 "면접" 내부 15쌍만 따로 보면: **13쌍이
  0.00~0.20**(예: "자료구조 면접" ↔ "DB 면접" = 0.00, "일반 면접"
  ↔ "자료구조 면접" = 0.00). 높은 것도 2쌍뿐(0.85, 0.70).
- 반대로 사용자가 "판단 보류"라고 한 GitHub repo(spring-framework)와
  "면접총정리" 글이 0.90으로 매칭 - 실제 내용을 보니 둘 다 Spring/Java
  얘기가 실제로 많이 겹쳐서 나온 설명 가능한 고점수였다(할루시네이션
  아님).

### Observation
"면접 항목끼리 안 묶이는 게 Clique-safe 순서 문제 아닐까"라는
가설을 raw score로 직접 반박했다 - Mechanism 판단 자체가 "자료구조
면접"과 "DB 면접"을 서로 다른 구체적 대상으로 정확히 구분하고
있었다(프롬프트가 지시한 대로: "같은 상위 분야라는 이유만으로 높은
점수를 주면 안 된다"). 즉 그룹핑 알고리즘 이전에, **애초에 비교
기준(구체적 대상 동일성) 자체가 사용자가 원하는 관리 단위(같은
상위 목적: 취업 준비)와 다르다**는 게 확인됨. 동시에, 처음
"오매칭"으로 봤던 사례(GitHub repo ↔ 면접 글)도 다시 보니
할루시네이션이 아니라 "내용은 겹치지만 자료 유형/사용 목적이 다른"
케이스였다 - "판단이 틀렸다"가 아니라 "판단이 안 보는 축(자료 유형,
저장 목적)이 있다"는 쪽에 더 가깝다.

### Decision
기존 Finding A/C를 아래처럼 재정의한다(과거 스냅샷은 안 고치고
이어서 기록 - Phase 1/2 요약 문서 때와 같은 원칙).

- **Finding A(핵심) - Topic granularity mismatch**: Mechanism
  Judge는 "같은 구체적 대상/기법인가"를 판단하도록 설계돼 있고,
  사용자가 원하는 건 "관리 단위(폴더/관심사)" - 판단 목적 자체가
  다르다. 사례: RAG 개념/실전, 면접/Spring 면접/DB 면접. 이번
  실험으로 "그룹핑을 개선하면 풀린다"가 아니라 "그룹핑 이전에 목표
  단위 자체가 다르다"는 게 확인됨 - 근본 원인으로 승격.
- **Finding B - Grouping algorithm limitation**: pairwise 관계는
  있는데 clustering 과정(Clique-safe 그리디 순서)에서 손실되는
  경우. 사례: 서울 관광 ↔ 경복궁(PR #85). 이번 개발 CS 데이터에서는
  재현 안 됨(raw score 자체가 낮아서 애초에 묶일 관계가 아니었음) -
  증거 부족, 추가 관찰 필요 상태 유지.
- **Finding C(재정의) - Context-blind similarity**: "Pairwise false
  positive"에서 이름 변경 - "오매칭"이라는 표현은 정답이 있다는
  뉘앙스라 부적절했다. Pairwise Judge는 문서 내용 기반 관련성만
  판단하고, 사용자가 중요하게 보는 관리 맥락(자료 유형, 사용 목적,
  저장 목적)은 애초에 안 본다. 사례: Spring 공식 repo ↔ Spring 면접
  준비 글(0.90, 내용은 실제로 겹침, 그러나 사용자는 "하나는
  프로젝트 코드, 하나는 학습 자료"로 분리하고 싶어함. **미사용
  레버 발견**: `Scrap.userContext`(스크랩 시 "왜 저장했나요?" 선택
  입력)가 스키마에 이미 있는데 `TopicCandidateService`의 Mechanism
  비교는 `summary`만 쓰고 `userContext`는 전혀 안 씀 - 나중에 이
  Finding을 풀 때 새 인프라 없이 시도해볼 수 있는 첫 후보로 기록만
  해둠, 지금 구현 안 함.

**다음 액션 없음**: "후보 merge"(PR #88)와 "AI 그룹 참고하되 사용자가
최종 결정"(체크박스 자유 편집, 이미 존재)은 둘 다 이미 있는 기능이라
이번 라운드에서 새로 만들 것은 없음. 다음 관찰은 실사용 중 자연히
쌓이는 대로.

## Stage 1 스케일링 우려 + cosine Top-K 순위 실험 (2026-07-30)

개발 Island에 사용자가 직접 Topic 2개("알고리즘" 2개, "개발 면접" 6개)를
만들고 새 스크랩 6개를 추가한 뒤 실제로 "Topic 후보 찾기"를 돌려서 관찰.

### Setup
`/islands/1/topic-candidates` 실행. 신규 미분류 스크랩(73~78, 기존 2,3
포함) vs 기존 Topic 멤버 전원 비교(Stage 1 그대로).

### Result
- 7건이 기존 Topic에 매칭됨: 알고리즘 2/2(77→49 0.85, 78→49 0.85), 개발
  면접 5/6(73→36 0.85, 75→35 0.60, 76→35 0.60, 2→38 0.65, 3→38 0.90).
- 74번("📘 기술면접 질문 정리")만 미분류로 남음 - raw score를 직접
  까보니 실제로는 인성면접(지원동기 등) 위주 콘텐츠라 나머지 6개(전부
  기술면접)와 raw Mechanism이 0.0~0.3으로 일관되게 낮았음. 버그가
  아니라 진짜 다른 하위 주제.

### Observation
Stage 1은 Stage 2보다 실사용 정확도가 좋다 - 멤버 중 하나와만 강하게
겹치면 매칭되는 구조라 Clique-safe의 "전원 동의" 제약이 없어서 Finding
A의 영향을 덜 받음. 다만 코드 구조상 **미분류 스크랩 1개당 그 Topic의
멤버 전부와 비교**하고 있어서(`TopicCandidateService` Stage 1),
Topic이 잘 자랄수록(=기능이 성공할수록) 비용이 무한정 커지는 구조임을
확인 - Stage 2의 O(n²)와 반대로 자연히 줄어들지 않음.

이어서 "멤버 전부와 비교할 필요가 있는가"를 확인하기 위해, 위 7건의
매칭에 대해 새 스크랩과 Topic 멤버 전체 간 cosine similarity를 로컬로
계산해서, Mechanism이 고른 실제 최고점 멤버가 cosine 순위로 몇 등인지
확인.

### Result 2
**7/7 전부 cosine 1위 = Mechanism 최고점 멤버와 일치**(예: 스크랩 73 vs
"개발 면접" 멤버 6개 - cosine 1위 0.651 = Mechanism 최고점 0.85 멤버와
동일; 스크랩 3 vs 6개 - cosine 1위 0.400 = Mechanism 최고점 0.90 멤버와
동일).

### Observation 2
표본이 매우 작다(한 Island, 하루, N=7) - Evidence가 아니라 Hypothesis
수준으로 취급해야 함. 다만 이전 cosine 실험(AI Island, 절대 threshold로
거르는 용도, Pearson 0.60)과는 **다른 질문**이라는 점이 중요 - 이번엔
"이 Topic 안에서 가장 관련 있는 멤버가 누구인가"라는 상대 순위 질문이고,
절대 점수의 노이즈가 상대 순위에서는 상쇄될 가능성이 있어서 결과가
훨씬 깔끔하게 나온 것으로 보임.

### Decision
지금 Topic이 아직 작아서(6~10개) 당장 코드를 바꾸지 않음. 근거만
기록해두고, Topic이 20~30개 이상으로 커진 뒤 표본을 늘려 재확인 -
가설이 유지되면 Stage 1을 "embedding → cosine Top-K → Mechanism"으로
재설계할 근거가 됨. 전체 정리는 `docs/v2_architecture_review.md` 참고.

## Cosine Top-K 가설 기각 - "여행" Island 반례 (2026-07-30)

위 실험(N=7, 개발 CS)의 표본 크기 문제를 사용자가 직접 지적("실험
표본이 너무 적은거 아니야?") - 실질적 독립 시행이 2건(Topic 2개)뿐이고,
후보 풀도 2~6개로 작고, 도메인도 하나뿐이었다는 약점을 인정하고,
다른 도메인("여행")에서 훨씬 큰 후보 풀로 재검증.

### Setup
"여행" Island(22개, 231쌍) 전체를 Topic 경계 없이 다시 캡처(cosine +
Mechanism, gpt-4o-mini). Mechanism ≥ 0.6인 진짜 관련 쌍마다, 그
파트너가 나머지 21개 후보 전체 중 cosine 순위로 몇 등인지 확인(Topic
멤버 6개가 아니라 Island 전체 21개가 후보 풀 - 개발 CS보다 3~10배
큰 풀, 실제 음성 사례도 자연히 섞임).

### Result
Mechanism≥0.6인 쌍 9개 중: cosine 1등 4/9(44%), 3등 이내 7/9(78%),
5등 이내 8/9(89%). **반례 1건**: "부산광역시/관광" ↔ "부산 광안리
여행 코스 추천"이 Mechanism 0.85(강한 관계)인데 cosine 순위는
**8등/21개** - Top-5 shortlist였어도 이 관계는 빠졌을 것.

### Observation
개발 CS(N=7)의 "7/7 전부 cosine 1등"은 후보 풀이 작고(2~6개) 같은
Topic·같은 도메인·비슷한 문서들이라는 조건에서 우연히 깨끗하게 나온
결과였다. 후보 풀을 21개로 늘리고 도메인을 바꾸자 바로 강한 반례가
나옴 - 표본이 작을 때의 낙관적 신호가 일반화되지 않는다는 걸 직접
확인. 사용자가 표본 크기를 문제 삼지 않았다면 이 반례를 놓치고
cosine Top-K를 그대로 구현했을 뻔함.

### Decision
**Finding: cosine ranking은 Stage 1의 안전한 shortlist로 쓰기
어렵다.** 여행 Island(21개 후보)에서 Mechanism 0.85인 실제 관련
쌍이 cosine 8위를 기록한 반례가 확인됨 - cosine Top-K 기반 후보
축소를 Stage 1의 일반적 최적화 전략으로 채택하지 않는다.

질문 자체를 다시 본다 - "Stage 1을 어떻게 빠르게 할까"가 아니라
"Topic을 어떻게 대표할까"가 진짜 질문일 수 있음(새 스크랩을 Topic
멤버 전원과 비교하는 대신, Topic의 대표성만으로 비교하는 방향).
다만 이 방향도 지금 바로 시도하지 않는다 - V0 `prototype/world.py`의
`Topic.center_vector`(평균 임베딩 centroid) 시도가 작은 표본에서
embedding noise가 그대로 드러나 실패했던 전례(Finding #005/#006,
Night Batch 자동 재구성 실패 원인 중 하나)가 있어서, 같은 함정을
반복하지 않으려면 먼저 근거부터 쌓아야 함. 지금은 Architecture
Review 문서에 "왜 느려질 수 있는지 / 왜 cosine Top-K를 기각했는지 /
아직 검증된 대안이 없다는 것"만 기록하고, Topic이 실제로 20~30개
이상으로 커졌을 때 데이터로 재접근한다.

## "바이오" Island 부트스트랩 - 40개 규모 실측 + 설계 원칙 정리 (2026-07-30)

지금까지 본 것 중 가장 큰 규모(40개, Topic 없음)로 "Topic 후보 찾기"를
실제로 돌려서 (a) 대규모에서 소요 시간이 어떻게 나오는지 (b) 그룹핑
품질이 사용자 기준과 얼마나 맞는지 관찰.

### Setup
`/islands/6/topic-candidates` 실행(40개, C(40,2)=780쌍, PARALLELISM=2,
Topic 없어서 전부 Stage 2 - 신규 그룹 발견).

### Result
- **소요 시간 7분 17초(437초)** - 쌍당 ~0.56초. 이전 "여행" 실측(276쌍/
  83.55초, 쌍당 ~0.30초)보다 느림 - 규모가 커질수록 TPM 재시도가 더
  자주 걸리는 것으로 추정. 이 실측 기준으로 재외삽하면 50개 약 11분,
  80개 약 30분 - 기존 Architecture Review에 적은 외삽치(50개 7분,
  80개 16분)보다 나쁨.
- 11개 그룹(2~5개씩) + 8개 미분류. "바이오의약품이 뭔가"를 설명하는
  개념 글이 그룹1(5개)/그룹8(2개)/그룹11(5개) **세 군데**로 흩어짐 -
  AI Island의 RAG 개념/실전 분리와 같은 패턴이 40개 규모에서도 재현.

### Observation
사용자에게 "이 그룹핑이 원하는 만큼 잘 나뉘었냐"고 물었더니, "GMP
교육은 따로 할 것 같고... 바이오의약품도 정의/종류로 나눌지 합칠지
모르겠다"는 반응 - **AI Island의 "RAG는 하나로 묶고 싶다"는 확신과
달리, 이번엔 사용자 본인도 적정 Topic 크기를 바로 결정하지 못함.**
즉 "정답인 Topic 단위"가 항상 고정적으로 존재하는 게 아니라, 사람도
같은 사람도 상황(공부용/시험 준비용 등)에 따라 다르게 관리하고
싶어할 수 있다는 것.

이걸 현재 시스템 제약과 같이 보면 설계적으로 의미가 생긴다 -
`TopicController`를 확인한 결과 **Topic 생성/추가(merge 방향)는
있지만 Topic에서 스크랩 제거나 Topic 분할(split 방향)은 API 자체가
없음.** 그러니 처음부터 크게 하나로 묶어버리면 나중에 "정의/종류/
생산공정"으로 다시 나누고 싶어도 지금 시스템으로는 되돌릴 방법이
없고, 반대로 작게 제안해두면 사용자가 필요할 때 "이 후보 추가"로
언제든 합칠 수 있음(PR #88, 이미 검증됨).

### Decision
Finding A(granularity mismatch)를 "문제"가 아니라 설계 방향으로
재해석한다 - **AI는 "최종 Topic"이 아니라 "merge 가능한 최소 단위"를
제안하는 역할**로 본다. 하나의 진짜 주제를 여러 개의 작은 후보로
쪼개는 것 자체는 되돌릴 수 있어서 안전하지만(사용자가 언제든 합침),
서로 다른 주제를 하나로 잘못 묶는 것(Finding C류 오매칭, 예:
RAG↔프롬프트)은 여전히 줄여야 할 문제로 남는다 - 이 둘을 같은 것으로
취급하지 않는다. 코드 변경 없음 - 현재 "AI 제안(작게) → 사용자
merge(필요하면)" 구조를 유지할 근거가 하나 더 쌓임. 전체 정리는
`docs/v2_architecture_review.md` 참고.

## "경제, 재테크" Island 실사용 - 새 Extraction 실패 유형 + 가짜 1.00 매칭 (2026-08-01)

MapView/TopicMapView 완성 후 실사용 관찰 중, 사용자가 "경제, 재테크"
Island에 카드/예적금 비교 사이트 6개를 스크랩하고 "Topic 후보 찾기"를
돌렸는데 **하나도 안 묶임**("적어도 카드 비교라는 걸로는 생겨야
하는데?"). 이어서 2개를 더 스크랩(뱅크샐러드, 예적금 비교 다른 탭)해서
재현성까지 확인.

### Setup
`/api/islands/7/topic-candidates` 실행(8개, C(8,2)=28쌍) 후, 결과가
비어서 raw Mechanism 점수와 실제 요약 내용을 직접 확인(Java 앱 우회,
Python으로 직접 캡처 - 기존 진단 패턴 재사용).

### Result
- **groups: [], existingTopicMatches: [] - 전부 미분류.**
- 카드 비교 3개(121 신용카드비교, 122 신용카드추천, 124 카드사별검색)
  끼리 raw Mechanism = **0.20**(threshold 0.6에 한참 못 미침) -
  제목만 보면 명백히 같은 주제인데도.
- 실제 요약 내용 확인: 121/122/125/127은 `DIRECT_EXTRACTION`으로
  "성공" 표시돼 있지만 실제 내용은 진짜 카드/적금 목록이 아니라
  **법적 고지문("네이버파이낸셜㈜은 전자결제서비스제공자..."), 약관
  안내, 사업자 정보, 마케팅 헤드라인**뿐. 124는 `OPEN_GRAPH_ONLY`로
  이미 정직하게 폴백됨(51자, 정상 동작).
- **125 ↔ 128: Mechanism = 정확히 1.00.** 서로 다른 상품 페이지
  (예금 vs 적금, 같은 네이버페이 사이트의 다른 탭)인데 추출된
  텍스트가 완전히 동일(같은 법적 고지 푸터)해서 최고점 매칭이 됨 -
  실제로는 무관한 두 페이지.

### Observation
두 가지 서로 다른 문제가 겹쳐 있다.

1. **새로운 Extraction 실패 유형**: 금융상품 비교/검색 UI 페이지
   (카드/적금 리스트)는 실제 콘텐츠가 JS로 동적 렌더링되는 것으로
   보이고, jsoup 정적 파싱은 그 앞단의 "정적 골격"(약관/고지문/
   헤드라인)만 긁어온다. 기존 가드레일(`ExtractionQualityEvaluator`
   - 본문 50자 미만 폴백, 이커머스 운영 어휘 밀도 체크)은 둘 다 이
   케이스를 못 잡는다 - 텍스트가 충분히 길고(700자+) 운영 어휘도
   안 섞여 있어서. V1의 "SPA 랜딩페이지 껍데기"(PR #69), "이커머스
   운영 문구"(PR #75) Finding과 같은 계열이지만 새로운 소스 타입.
2. **Finding C(Context-blind similarity)의 더 극단적인 사례**: 이전
   RAG↔프롬프트(0.65), Spring repo↔면접글(0.90)은 "설명 가능한 오매칭
   "이었는데, 이번엔 완전히 동일한 boilerplate 텍스트로 인한 **완벽한
   가짜 매칭(1.00)**. 실제 내용이 겹쳐서가 아니라 추출 실패의 부산물이
   우연히 똑같아서 생긴 것 - Mechanism 판단 자체는 "텍스트가 똑같으니
   같은 대상"이라고 정확히 답한 거라 알고리즘 오류는 아님.

**userContext 관련 정정(사용자가 직접 반박, 재확인함)**: 처음에
"8개 전부 userContext가 비어있다"고 답했는데 틀렸음 - 그건 127/128이
생기기 전(6개 시점) 쿼리 결과를 8개 전부에 성급하게 일반화한 것.
재확인 결과 127/128은 실제로 "어느 적금이 좋은지 비교를 해보기
위해서"가 정확히 채워져 있었음. 즉 **사용자는 의도를 실제로 입력했고,
문제는 순전히 `TopicCandidateService`가 `userContext`를 코드에서
아예 안 읽는다는 것 하나뿐** - 사용자 습관 문제가 아니라 엔지니어링
갭. 답을 재확인 없이 이전 쿼리 결과로 일반화한 것 자체가 이 프로젝트가
반복 강조해온 "확인 후 판단" 원칙 위반 사례로 남겨둠.

**"summary"는 LLM 요약이 아님(확인)**: `ScrapContentPreprocessor.
truncate()`는 `content.substring(0, maxLength)` - 순수 문자열
자르기. 콘텐츠 품질을 LLM이 판단하는 단계는 파이프라인에 전혀 없음.
토큰 비용 때문이 아님 - 스크랩 1개당 추가 LLM 호출 1번(이미 임베딩
호출이 1번 있음, O(1) - pairwise judging의 O(n²)와 다름)이라 비용은
낮음. V1 Content Extraction(PR #49~55, 실측 88% 성공률) 설계 당시
"추출된 텍스트면 다음 단계에 넘길 만큼 충분하다"는 가정이 새 소스
타입에서 처음 깨진 사례로 해석.

### Decision
지금 당장 코드 수정 안 함. Finding으로 기록만 하고, 다음 실험으로
"LLM이 추출된 텍스트가 진짜 콘텐츠인지 boilerplate인지 판별할 수
있는가"를 이 8개 스크랩(이미 사람이 직접 읽어서 정답 라벨을 확인함
- 120/126은 진짜 콘텐츠, 121/122/124/125/127/128은 boilerplate)으로
먼저 싸고 작게 검증한다. "LLM 요약을 Mechanism 비교 입력으로 쓰는
것"과는 다른 질문으로 명시적으로 분리 - 후자는 애초에 추출 안 된
콘텐츠(JS 렌더링 부분)를 요약이 만들어낼 수 없어서 근본 해법이 아님.

## Content-quality 분류 실험 - 소표본 검증 후 전체 스윕 (2026-08-01)

위에서 계획한 대로, 8개(경제/재테크) → 4개(패션) → 전체 120개
순서로 표본을 늘려가며 "LLM이 boilerplate를 판별할 수 있는가"를
검증. gpt-4o-mini, temperature 0, CONTENT/BOILERPLATE 이진 분류
프롬프트 하나로 통일.

### Setup
1차: 경제/재테크 8개(사람이 직접 읽어서 정답 라벨 확정: 120/126만
CONTENT, 나머지 6개 BOILERPLATE).
2차: 패션 4개(20/21/23 CONTENT, 22 무신사 상품페이지 BOILERPLATE로
사람이 라벨링).
3차: 전체 120개(정답 라벨 없이, island별 BOILERPLATE 비율만 집계)
- 실행 42.3초(PARALLELISM 2).

### Result
- 경제/재테크: 7/8 정확(87.5%). 유일한 실수(124번)는 "카드 검색"
  같은 구체적 명사가 들어간 마케팅 헤드라인을 CONTENT로 오분류.
- 패션: 3/4 정확(75%). 유일한 실수(22번 무신사)도 같은 패턴
  (상품명이 구체적으로 들어간 페이지를 CONTENT로 오분류) - **재현성
  문제도 발견**: 같은 22번이 전체 스윕(3차)에서는 반대로 CONTENT로
  나옴, 같은 텍스트·같은 프롬프트·temperature 0인데도 결과가 바뀜.
- 전체 120개 island별 BOILERPLATE 비율: 개발 0%(17개), 여행
  **13.6%**(3/22), 야구 0%(1개), 패션 0%(4개, 재현성 문제로 위
  결과와 다름), AI 0%(22개), 바이오 **7.5%**(3/40), 경제/재테크
  62.5%(5/8, 재현성으로 위 8/8과 살짝 다름), 미확정 0%(6개).
- **여행/바이오 6건을 직접 열어서 확인**: 5건(제주신화월드 나무위키,
  63빌딩, 서울관광공식사이트, 한국바이오인력개발센터, 한국제약바이오협회
  교육)은 실제로 진짜 boilerplate(저작권 고지/개인정보처리방침/
  쿠키동의/사업자정보뿐, 실제 소개 내용 전무) 확인됨. 1건(96
  제약바이오 용어사전)은 분류기 오판으로 판단 - 실제로는 진짜
  용어 목록(용어 사전 콘텐츠)인데 설명문이 아니라 나열 형식이라
  BOILERPLATE로 오분류된 것으로 보임.
- **`content`(원문 전체) vs `summary`(잘린 버전) 길이 비교**: 5건의
  진짜 boilerplate 전부 `content`와 `summary` 길이가 **완전히
  동일**(예: 63빌딩 136자=136자) - truncation 때문에 정보를 놓친
  게 아니라, jsoup이 애초에 그만큼만 추출했다는 뜻.

### Observation
사용자가 처음 낸 가설("다른 island는 boilerplate가 없어서 잘
됐던 것 아니냐")은 **부분적으로 틀림** - 개발/야구/AI는 실제로
0%였지만, 여행/바이오에도 boilerplate가 섞여 있었음(각각
13.6%/7.5%). 다만 비율이 낮아서 22개/40개 중 소수라 지금까지
눈에 안 띄었을 뿐. 이번에 발견된 유형은 지난번(금융비교/이커머스
UI, 도메인 특정적)과 다르게 **저작권 고지/개인정보처리방침/쿠키
동의/사업자정보 같은 범용 사이트 푸터·약관류**라 특정 도메인에
국한되지 않는 더 일반적인 패턴으로 보임.

LLM 판단의 가치도 두 가지로 명확히 나뉨을 재확인:
- **품질 감지(이 텍스트가 쓸모없는가)**: 유효함. 기존
  `ExtractionQualityEvaluator`(길이/이커머스 키워드 규칙)로는
  개인정보처리방침·저작권 고지 같은 걸 절대 못 잡는데 LLM은 잡음.
- **콘텐츠 복원(진짜 내용이 뭔지 재구성)**: 여전히 불가능.
  `content`=`summary` 길이가 같다는 게, jsoup이 처음부터 실제
  콘텐츠에 도달하지 못했다는 뜻 - 없는 정보를 요약이 만들어낼 수
  없음.

### Decision
Finding으로 기록. LLM 기반 품질 게이트(기존 규칙 기반
`ExtractionQualityEvaluator`를 보완/대체)는 근거가 쌓였다고 판단 -
다만 재현성 문제(같은 항목이 실행마다 다르게 분류됨, N=12에서
직접 확인)가 있어서 단일 호출로 바로 파이프라인에 넣기보다 추가
검증이 필요. 콘텐츠 복원(LLM 요약을 비교 입력으로 쓰는 것)은
여전히 근거 없음 - 채택 안 함. 다음 질문: 정적 파싱(jsoup)이
못 가져오는 콘텐츠를 실제 브라우저 렌더링(Playwright 등)으로
가져올 수 있는지는 별도로 확인 필요(과거 "증거 없어서 도입 안
함" 결정을 새 증거로 재검토하는 것).

## 실제 브라우저 렌더링 검증 - jsoup이 놓친 콘텐츠가 실제로 있는가 (2026-08-01)

V1 Extraction Validation 단계(PR #55)에서 "Playwright/헤드리스
브라우저 도입 안 함 - 증거 없음"으로 결정했던 걸, 오늘 나온 실패
사례로 재검토할 가치가 있는지 확인. Java 앱/jsoup을 완전히 우회하고,
claude-in-chrome으로 실제 브라우저를 열어 같은 URL의 렌더링 결과를
jsoup 추출 결과와 직접 비교.

### Setup
boilerplate로 확인된 두 URL을 실제 브라우저로 열어서 `get_page_text`
결과와 DB에 저장된 jsoup 추출 결과(`content`)를 비교:
1. 63빌딩(scrap 34, jsoup 추출 136자 - 주소/사업자등록번호/저작권뿐)
2. 뱅크샐러드 적금 차트(scrap 127, jsoup 추출 309자 - 회사소개/
   사업자정보뿐)

### Result
- **63빌딩**: 실제 렌더링 시 "63빌딩 — 일상이 예술이 되는 곳", "퐁피두센터
  한화", "63 스카이 피크닉 전망대", "63 뷔페 파빌리온" 등 실제 시설/
  브랜드/공지사항이 전부 나옴 - jsoup이 가져온 136자와는 완전히 다른
  풍부한 콘텐츠.
- **뱅크샐러드**: 실제 렌더링 시 "NH청년도약계좌 최고 6%/기본 4.5%",
  "신한 청년도약계좌", "우리은행 청년도약계좌" 등 실제 적금 상품
  목록·금리 정보가 전부 나옴 - jsoup이 가져온 309자(회사소개+
  사업자정보)와는 완전히 다름.
- 2/2 - jsoup이 놓친 실제 콘텐츠가 두 경우 모두 실존함을 직접 확인.
- (참고) `card-search.naver.com`은 브라우저 자동화 안전 정책상 접근
  차단됨 - 네이버 계열 사이트라 추정, 검증 대상에서 제외.

### Observation
두 사례 모두 페이지가 100% JS로 렌더링되는 구조였고, jsoup(정적
HTML 파싱)은 그 앞단의 정적 골격(주소/사업자정보/저작권 표시)만
가져왔다는 게 실제 렌더링 비교로 명확히 확인됨. "헤드리스 브라우저를
쓰면 이 문제가 해결될 것"이라는 게 이제 추정이 아니라 직접 확인한
사실.

### Decision
Playwright 등 헤드리스 브라우저 도입을 재검토할 근거가 실제로
쌓였다고 기록만 해두고, 실제 도입 여부(속도/리소스/배포 복잡도
트레이드오프 포함한 설계)는 다음 세션으로 미룸 - 오늘은 실사용
관찰과 진단만 하고 코드 변경은 하지 않기로 함.

## 오늘의 실사용 관찰 요약 (2026-08-01)

MapView/TopicMapView 완성 후 실사용 중 "경제, 재테크" Island에서
시작된 연쇄 발견. 코드 변경은 전혀 없었고 전부 진단/실험만 진행:

1. Topic 후보가 하나도 안 묶임 → raw Mechanism 점수 확인 →
   그룹핑/판단 문제가 아니라 Extraction 실패로 확인(새로운 소스
   유형: 금융상품 비교/검색 UI 페이지).
2. 서로 다른 상품 페이지가 boilerplate 중복으로 가짜 1.00 매칭
   (Finding C의 더 극단적인 사례).
3. userContext 관련 자기 정정 - 사용자가 실제로 입력했는데 코드가
   안 읽는 것뿐(사용자 습관 문제 아님, 순수 엔지니어링 갭).
4. "summary"가 LLM 요약이 아니라 순수 truncation이라는 것 확인,
   토큰 비용은 진짜 이유가 아님도 확인.
5. LLM 기반 content-quality 분류 실험(8→4→120개로 표본 확대) -
   여행/바이오에도 몰랐던 boilerplate가 있었음을 발견(범용 법적
   고지/개인정보처리방침류, 도메인 특정적이지 않음). 재현성 문제도
   같이 발견.
6. 실제 브라우저 렌더링으로 jsoup이 놓친 콘텐츠가 실존함을 직접
   확인(2/2) - Playwright 재검토 근거 마련.

**다음 세션 결정 필요 사항**: 이 근거들을 갖고 어떻게 나아갈지
(Playwright 도입 여부/범위, LLM 품질 게이트 도입 여부, userContext를
Mechanism에 연결할지) - 오늘은 결정 안 하고 다음으로 미룸.
